...
Mat4 _ainline
ViewMatRHS(Vec3 position,Vec3 lookpoint,Vec3 updir){
    ...
    b = -1.0f * DotVec3(up,position),
    c = DotVec3(forward,position);
...
Mat4 QuatToMat4(Quat quaternion){
    ...
    f32 b = ((quaternion.x * quaternion.y) - (quaternion.w * quaternion.z)) * 2.0f;
    f32 c = ((quaternion.x * quaternion.z) + (quaternion.w * quaternion.y)) * 2.0f;
    
    
    
    f32 d = ((quaternion.x * quaternion.y) + (quaternion.w * quaternion.z)) * 2.0f;
    f32 e = 1.0f - (2.0f * (squared.x + squared.z));
    f32 f = ((quaternion.y * quaternion.z) - (quaternion.w * quaternion.x)) * 2.0f;
    
    
    f32 g = ((quaternion.x * quaternion.z) - (quaternion.w * quaternion.y)) * 2.0f;
    f32 h = ((quaternion.y * quaternion.z) + (quaternion.w * quaternion.x)) * 2.0f;
...
Quat Mat4ToQuat(Mat4 matrix){
    ...
    if(trs > 0.0f){
        ...
        q.w = 0.25f * s;
        q.x = (matrix.container[_rc4(1,2)] - matrix.container[_rc4(2,1)])/s;
        q.y = (matrix.container[_rc4(2,0)] - matrix.container[_rc4(0,2)])/s;
        q.z = (matrix.container[_rc4(0,1)] - matrix.container[_rc4(1,0)])/s;    
    }
    
    else if((matrix.container[_rc4(0,0)] > matrix.container[_rc4(1,1)]) &&
		    (matrix.container[_rc4(0,0)] > matrix.container[_rc4(2,2)])){

        f32 s = sqrtf(1.0f + matrix.container[_rc4(0,0)] - matrix.container[_rc4(1,1)] - 
			matrix.container[_rc4(2,2)]) * 2.0f;
        q.w = (matrix.container[_rc4(1,2)] -  matrix.container[_rc4(2,1)]) /s;
        q.x = 0.25f * s;
        q.y = (matrix.container[_rc4(1,0)] + matrix.container[_rc4(1,0)])/s;
        q.z = (matrix.container[_rc4(2,0)] + matrix.container[_rc4(0,2)])/s;        
    }
    
    else if(matrix.container[_rc4(1,1)] > matrix.container[_rc4(2,2)]){
        f32 s = sqrtf(1.0f + matrix.container[_rc4(1,1)] - matrix.container[_rc4(0,0)] - 
			matrix.container[_rc4(2,2)]) * 2.0f;

        q.w = (matrix.container[_rc4(2,0)] -  matrix.container[_rc4(0,2)]) /s;
        q.x = (matrix.container[_rc4(1,0)] + matrix.container[_rc4(1,0)])/s;
        q.y = 0.25f * s;
        q.z = (matrix.container[_rc4(2,1)] + matrix.container[_rc4(1,2)])/s;            
    }
    
    else{
        f32 s = sqrtf(1.0f + matrix.container[_rc4(2,2)] - matrix.container[_rc4(0,0)] - 
			matrix.container[_rc4(1,1)]) * 2.0f;

        q.w = (matrix.container[_rc4(0,1)] -  matrix.container[_rc4(1,0)]) /s;
        q.x = (matrix.container[_rc4(2,0)] + matrix.container[_rc4(0,2)])/s;
        q.y = (matrix.container[_rc4(2,1)] + matrix.container[_rc4(1,2)])/s;
        q.z = 0.25f * s;
    }
...
Mat4 Mat4Add(Mat4 lhs,Mat4 rhs){
    ...
    _mm_store_ps(&matrix.container[_ac4(0,0)],_mm_add_ps(lhs.simd[0],rhs.simd[0]));
    
    _mm_store_ps(&matrix.container[_ac4(0,1)],_mm_add_ps(lhs.simd[1],rhs.simd[1]));
    
    _mm_store_ps(&matrix.container[_ac4(0,2)],_mm_add_ps(lhs.simd[2],rhs.simd[2]));
    
    _mm_store_ps(&matrix.container[_ac4(0,3)],_mm_add_ps(lhs.simd[3],rhs.simd[3]));
...
Mat4 Mat4Sub(Mat4 lhs,Mat4 rhs){
    ...
    _mm_store_ps(&matrix.container[_ac4(0,0)],_mm_sub_ps(lhs.simd[0],rhs.simd[0]));
    
    _mm_store_ps(&matrix.container[_ac4(0,1)],_mm_sub_ps(lhs.simd[1],rhs.simd[1]));
    
    _mm_store_ps(&matrix.container[_ac4(0,2)],_mm_sub_ps(lhs.simd[2],rhs.simd[2]));
    
    _mm_store_ps(&matrix.container[_ac4(0,3)],_mm_sub_ps(lhs.simd[3],rhs.simd[3]));
...
Mat4 Mat4MulConstR(Mat4 lhs,f32 rhs){
    return Mat4MulConstL(rhs,lhs);
}

Mat4 Mat4Div(Mat4 lhs,Mat4 rhs){
    
    return lhs * InverseMat4(rhs);
}
...
Mat3 Mat3Mul(Mat3 lhs,Mat3 rhs){
    ...
    matrix.container[_rc3(0,0)] = ((f32*)&h)[0] + ((f32*)&h)[1] + ((f32*)&h)[2];
    matrix.container[_rc3(1,0)] = ((f32*)&i)[0] + ((f32*)&i)[1] + ((f32*)&i)[2];
    matrix.container[_rc3(2,0)] = ((f32*)&j)[0] + ((f32*)&j)[1] + ((f32*)&j)[2];
    
    matrix.container[_rc3(0,1)] = ((f32*)&j)[3] + ((f32*)&k)[0] + ((f32*)&k)[1];
    matrix.container[_rc3(1,1)] = ((f32*)&h)[3] + ((f32*)&l)[0] + ((f32*)&l)[1];
    matrix.container[_rc3(2,1)] = ((f32*)&i)[3] + ((f32*)&m)[0] + ((f32*)&m)[1];
    
    matrix.container[_rc3(0,2)] = ((f32*)&m)[2] + ((f32*)&m)[3] + ((f32*)&o)[0];
    matrix.container[_rc3(1,2)] = ((f32*)&k)[2] + ((f32*)&k)[3] + ((f32*)&o)[1];
    matrix.container[_rc3(2,2)] = ((f32*)&l)[2] + ((f32*)&l)[3] + ((f32*)&o)[2];
...
Mat3 Mat3MulConstR(Mat3 lhs,f32 rhs){
    return Mat3MulConstL(rhs,lhs);
}


Mat3 Mat3Div(Mat3 lhs,Mat3 rhs){
    return lhs * InverseMat3(rhs);
}
...
Mat4 InverseMat4(Mat4 matrix){
    ...
    */
    ...
    f32 first_row[] = {matrix.container[_ac4(0,0)],matrix.container[_ac4(1,0)],
    ...
    for(u32 y = 0; y < 4; y++){
        ...
        for(u32 x = 0; x < 4; x++){
            ...
            GetMinorMat((f32*)&matrix.container[0],4,x,y,(f32*)&minormatrix);
            
            adj__matrix.container[_ac4(x,y)] = GenericGetDeterminant((f32*)&minormatrix,3);
        }
    ...
    adj__matrix = CompMulMat4(adj__matrix,checkerboard__matrix);
...
Mat3 InverseMat3(Mat3 matrix){
    ...
    f32 first_row[] = {
        matrix.container[_ac3(0,0)],matrix.container[_ac3(1,0)]
    ...
    adj__matrix = CompMulMat3(adj__matrix,checkerboard__matrix);
...
Vec4 InterpolateVec4(Vec4 a, Vec4 b, f32 step) {
	Vec4 v = {_intrin_fmadd_ps(_mm_sub_ps(b.simd, a.simd), _mm_set1_ps(step), a.simd)};
	return v;
}
...
f32 CompVec3(Vec3 a,Vec3 b){
    return DotVec3(a,NormalizeVec3(b));
}
...
Vec3 QuatRotateVec3(Vec3 v,Quat q){
    ...
    */
    ...
    auto r = (2.0f * DotVec3(q_v,v) * q_v) + (((q_w * q_w) - DotVec3(q_v,q_v)) * v) + 
...
f32 Cosf(Vec3 vec1,Vec3 vec2){
    return DotVec3(vec1,vec2)/(MagnitudeVec3(vec1) * MagnitudeVec3(vec2));
}
...
Quat InverseQuat(Quat q){
	...
	/*
	 The Inverse of the quaternion is defined as the ConjugateQuat(q)/(Magnitude(q))^2
	 However, as the quaternions used are unit quaternions, the Magnitude(q) is always 1. Just insert the conjugate here.
	 */
...
Quat InterpolateQuat(Quat a, Quat b,f32 step){
	Quat q = {_intrin_fmadd_ps(_mm_sub_ps(b.simd, a.simd), _mm_set1_ps(step), a.simd)};
	return q;
}
...
b32 TypedIntersectLine3(Line3 a,Line3 b){
    ...
    auto dir = NormalizeVec3(b.pos - a.pos);//checks if the lines are the same
    
    b32 res =
        (u32)(fabsf(DotVec3(NormalizeVec3(a.dir),dir)) + _f32_error_offset) +
        IntersectLine3(a,b);
...
b32 TypedIntersectLine2(Line2 a_2,Line2 b_2){
    ...
    Line3 a = {Vec3{a_2.pos.x,a_2.pos.y,1.0f},Vec3{a_2.dir.x,a_2.dir.y,1.0f}};
    Line3 b = {Vec3{b_2.pos.x,b_2.pos.y,1.0f},Vec3{b_2.dir.x,b_2.dir.y,1.0f}};
...
Mat4 WorldMat4V(Vec3 position,Vec3 rotation,Vec3 scale){
    ...
    matrix = WorldMat4M(position__matrix4,rotation__matrix4,scale__matrix4);
...
DualQuat ConstructDualQM(Mat4 transform){
    ...
    d.q2 = Quat{0,translation.x,translation.y,translation.z} * d.q1 * 0.5f;
